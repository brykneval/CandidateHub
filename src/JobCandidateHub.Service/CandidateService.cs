...
namespace JobCandidateHub.Service
{
	public class CandidateService : ICandidateService
	{
		...
		public async Task<EnumHelpers.EOpResult> AddOrUpdateCandidate(Candidate candidate)
		{
			...
			if (oCandidate == null)
			{
				dbStatus = await _candidateRepository.SaveCandidate(candidate);
			}
			else
			{
				dbStatus = await _candidateRepository.UpdateCandidate(oCandidate);
			}
